...
function TextChatBubble({
  ...
  // Copy full message text. Visible once the bubble has content and is
  // no longer streaming — mirroring the edit-button ergonomics.
  const [copied, setCopied] = useState(false);
  const canCopy = !editing && !bubble.streaming && bubble.content.length > 0;
  const copyMessage = useCallback(() => {
    navigator.clipboard.writeText(stripTimestamp(bubble.content)).then(() => {
      setCopied(true);
